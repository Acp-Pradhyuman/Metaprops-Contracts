...
async function deploy() {
  ...
  let factory = await ethers.getContractFactory("NFT12")
...
describe("NFT12", function() {
  it("Should deploy", async function() {
    const signers = await ethers.getSigners();
    const minter = signers[0].address;

    const LazyNFT = await ethers.getContractFactory("NFT7");
  ...
  it("Should redeem an NFT from a signed voucher", async function() {
    ...
    await expect(redeemerContract.createAndBuyToken(voucher))
      ...
      await redeemerContract.createAndBuyToken(voucher)
  ...
  it("Should redeem if payment is >= minPrice", async function() {
    ...
    await expect(redeemerContract.createAndBuyToken(voucher, {value: minPrice}))
      .to.emit(contract, 'Transfer')  // transfer from null address to minter
      .withArgs('0x0000000000000000000000000000000000000000', minter.address, voucher.tokenId)
      .and.to.emit(contract, 'Transfer') // transfer from minter to redeemer
      .withArgs(minter.address, redeemer.address, voucher.tokenId)
  ...
  it("Should fail to redeem if payment is < minPrice", async function() {
    ...
    // redeemerContract.createAndBuyToken(voucher, 5, '0x0000000000000000000000000000000000000000', {value: payment})
    await expect(redeemerContract.createAndBuyToken(voucher, {value: payment}))
  ...
  it("Should make payments available to minter for withdrawal", async function() {
    ...
    await expect(await redeemerContract.createAndBuyToken(voucher, {value: minPrice}))
  ...
  it("Should make payments available to minter for withdrawal", async function() {
    ...
    await redeemerContract.createAndBuyTokenWETH(voucher, token.address)
    ...
    await redeemerContract.start(3, minPrice, 10, token.address)

    await connectedWalletToken.approve(contract.address, '2000000000000000000')
    await connectedWalletContract.bid(3, '2000000000000000000')

    await connectedWalletToken2.approve(contract.address, '10000000000000000000')
    await connectedWalletContract2.bid(3, '10000000000000000000')
    ...
    await redeemerContract.end(voucher, '0x0000000000000000000000000000000000000000')

    await connectedWalletContract.withdraw(3)
    ...
    // await expect(await connectedWalletContract.createAndBuyToken(voucher, 5, {value: minPrice}))
