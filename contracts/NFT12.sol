...
contract NFT12 is ERC721URIStorage, EIP712 {
    ...
    function createAndBuyToken(NFTVoucher calldata voucher)
        public
        payable
        returns (uint256)
    {
    ...
    function createAndBuyTokenWETH(NFTVoucher calldata voucher, IERC20 _token)
        public
        payable
        returns (uint256)
    {
        ...
        // );
        require(msg.sender != signer, "Sellers can't buy from marketplace");
        uint256 allowToPayAmount = _token.allowance(msg.sender, address(this));
        require(
            allowToPayAmount >= voucher.minPrice,
    ...
    function _transferBuy(
        ...
        if (allMarketItems[_marketItemId].token == IERC20(address(0))) {
            admin.transfer(calculateCommission(_amount)); //Transfering the listing fee to the admin of the marketplace.
            allMarketItems[_marketItemId].seller.transfer(
                calculateSellerShare(_amount)
            ); //transferring the sale amount to the seller after deducting platform commission.
        } else {
            IERC20 token = allMarketItems[_marketItemId].token;
            token.transferFrom(_recipient, address(this), _amount);
            token.transfer(admin, calculateCommission(_amount));
            token.transfer(
    ...
    function start(
        ...
        started[_nftId] = true;
        endAt[_nftId] = block.timestamp + (auctionPeriodInDays * 1 days); //1 seconds
    ...
    // function endWithBidder(NFTVoucher calldata voucher, address bidder) external {
    //     require(bidder != address(0), "Invalid address");
    //     require(bids[voucher.tokenId][bidder] > 0, "Bidder not found");
    ...
    //     if (auctionPurchaseToken[voucher.tokenId] != address(0)) {
    //         IERC20 paymentToken = IERC20(auctionPurchaseToken[voucher.tokenId]);
    //         paymentToken.transfer(
    //             admin,
    //             calculateCommission(bids[voucher.tokenId][bidder])
    //         );
    //         paymentToken.transfer(
    //             signer,
    //             calculateSellerShare(bids[voucher.tokenId][bidder])
    //         );
    ...
    //     } else {
    //         admin.transfer(calculateCommission(bids[voucher.tokenId][bidder]));
    ...
    // function endWithHighestBidder(NFTVoucher calldata voucher) external {
    //     require(started[voucher.tokenId], "You need to start first!");
    //     // require(
    //     //     block.timestamp >= endAt[voucher.tokenId],
    //     //     "Auction is still ongoing!"
    ...
    //         paymentToken.transfer(
    //             admin,
    //             calculateCommission(highestBid[voucher.tokenId])
    //         );
    //         paymentToken.transfer(
    //             signer,
    //             calculateSellerShare(highestBid[voucher.tokenId])
    //         );
    ...
    //     } else {
    //         admin.transfer(calculateCommission(highestBid[voucher.tokenId]));
    //         payable(signer).transfer(
    //             calculateSellerShare(highestBid[voucher.tokenId])
    //         );
    ...
    //     emit End(voucher.tokenId, highestBidder[voucher.tokenId], highestBid[voucher.tokenId]);
    // }
    ...
    function calculateCommission(uint256 amount) public view returns (uint256) {
        uint256 commission = (platformCommission * amount) / 100;
        return commission;
    }

    function calculateSellerShare(uint256 amount)
        public
        view
        returns (uint256)
    {
        uint256 sellerShare = ((100 - platformCommission) * amount) / 100;
        return sellerShare;
    }
}
